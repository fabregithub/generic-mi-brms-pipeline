...
# ===================== CENSORED-EXPOSURE EXAMPLE SETTINGS =====================
# Demonstrates strategy = "censored_exposure_block_fcs" with TWO censored
# exposures: expo1 (single left-censoring) and expo2 (three-tier ND + DNQ
# interval). Each exposure carries leftcens interval columns X_lo / X_hi. The
# X-block conditions on the outcome Y (congeniality); MID drops imputed-Y rows.
# See 00_censored_exposure.R and validation/PLAN_leftcensored_exposure_integration.md.
analysis_spec$data$raw_data_file <- "data/censored_exposure_example.rds"
analysis_spec$outcome$y_var  <- "y"
analysis_spec$outcome$family <- "gaussian"
analysis_spec$outcome$link   <- "identity"
analysis_spec$imputation$strategy <- "censored_exposure_block_fcs"
analysis_spec$imputation$m        <- 10          # small for the example
analysis_spec$imputation$maxiter  <- 5
analysis_spec$imputation$impute_y <- FALSE
analysis_spec$imputation$censored_exposure <- list(
  exposure_vars = c("expo1", "expo2"),
  predictors    = c("y", "z1", "z2"),            # reduced X-block predictor set (+ Y)
  outer_sweeps  = 3L,
  margin        = "shash",
  log_scale     = TRUE,
  lo_suffix     = "_lo",
...
analysis_spec$model$custom_formula <- brms::bf(y ~ log(expo1) + log(expo2) + z1 + z2)
