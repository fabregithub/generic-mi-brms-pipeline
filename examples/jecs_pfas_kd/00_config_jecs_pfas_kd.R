...
# ===================== JECS PFAS -> KAWASAKI DISEASE EXAMPLE =====================
# Seven prenatal PFAS, three-tier interval-censored (leftcens <pfas>_lo/_hi), a
# rare binary outcome (Kawasaki disease), imputed congenially (outcome-aware,
# skew-robust) via strategy = "censored_exposure_block_fcs". Synthetic data
# grounded in Lai 2025 + Iwata 2024 (manuscript_demo/PROVENANCE.md).
.pfas <- c("PFOA","PFOS","PFNA","PFDA","PFUnDA","PFHxS","PFTrDA")

analysis_spec$data$raw_data_file <- "data/jecs_pfas_kd_example.rds"
analysis_spec$outcome$y_var  <- "kd"
analysis_spec$outcome$family <- "bernoulli"
analysis_spec$outcome$link   <- "logit"

analysis_spec$imputation$strategy <- "censored_exposure_block_fcs"
analysis_spec$imputation$m        <- 10          # small for the example; raise for real use
analysis_spec$imputation$maxiter  <- 5
analysis_spec$imputation$impute_y <- FALSE
analysis_spec$imputation$censored_exposure <- list(
  exposure_vars = .pfas,          # all 7 PFAS are censored exposures
  # Reduced X-block predictor set (PLAN §5): the OTHER PFAS (strong rescue via
  # correlation) + the outcome (congeniality) + key determinants. Deliberately NOT
  # the full covariate set — region alone is 14 dummies and the X-block cost is
  # ~k^2, so a lean set keeps it fast at n ~ 25000. (predictors = NULL uses the
  # auto set: outcome + all use_in_model covariates + exposures.)
  predictors    = c("kd", "mat_age", "parity", "egfr", .pfas),
  outer_sweeps  = 3L,
  margin        = "shash",        # skew-aware (the copula margin)
  log_scale     = TRUE,           # impute on the log scale; formula log2()s the exposures
  lo_suffix     = "_lo", hi_suffix = "_hi",
  mid_delete_imputed_y = TRUE
)

# Mutually-adjusted logistic on log2 PFAS (per-doubling OR, as in Iwata) + confounders.
analysis_spec$model$custom_formula <- brms::bf(stats::as.formula(paste(
  "kd ~", paste(sprintf("log2(%s)", .pfas), collapse = " + "),
  "+ mat_age + parity + bmi + smoking + income + child_male + low_bw + region")))
analysis_spec$model$chains <- 2
analysis_spec$model$iter   <- 1000
analysis_spec$model$warmup <- 500

paths$raw_data <- analysis_spec$data$raw_data_file
