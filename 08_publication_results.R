...
safe_step("STEP 8: Publication-ready outputs", {
  ...
  # Checked at write time (not just eval=file.exists at render time) so
  # that the whole chapter -- including its intro/closing prose, not just
  # the individual table/figure chunks -- is omitted for models that never
  # ran 09/10 in the first place, rather than printing an explanatory
  # paragraph for a chapter with nothing underneath it.
  mo_cumulative_or_table_file <- file.path(pub_dir, "mo_effects", "tables", "mo_cumulative_or_table.csv")

  mo_effects_report_lines <- if (file.exists(mo_cumulative_or_table_file)) {
    c(
      "# Monotonic-effect (mo()) results",
      "",
      "This chapter reports category-specific odds ratios derived from monotonic-effect (`mo()`) parameters, generated by `09_check_mo_parameter_columns.R` and `10_publication_mo_results.R`.",
      "",
