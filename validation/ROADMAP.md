...
| **Shipped default is biased under a confounder or mediator** | Open — **documented 2026-09-07; cause still unattributed (V20 running)** | `pipeline_bartMI` with `Y` in both blocks — the recommended configuration — carries +4.96% (fork), +5.39% (pipe), +8.52% and +10.40% (both under MAR), against ≤2.3% in every precision cell. The oracle is unbiased in all four, so it is the imputation's bias, not the design's. **Coverage does not detect it** (0.93–0.97, width/SE 0.98–1.13) because at `n` = 800 the bias is only 0.35–0.67 empirical SE. V18 measured how it scales: **n^−1/3**, so it shrinks with data but never washes out (2.0–3.6% still present at n = 12,800). Now documented for users in `docs/covariate-roles.md`. **V20** (`phase1/run_v20_attribute.sh`, ~2.9 h) is the attribution: a 2×2 swapping each block's draw for the exact conditional, with `R/exact_fork.R` supplying conditionals that are correct under a fork — V12/V13's were not. |
...
| **Attribute the bias to a block** | ⚠️ **V20 built and registered 2026-09-07; run pending** | V17 found it, V18 measured its `n`-scaling, V19 ruled out the imputer-family account — but nothing has yet said whether it comes from the X block, the Z block or the alternation. All four V19 arms share the `leftcens` X block, yet three are flat and one decays, so a common X-block floor does not explain the pattern alone. V20 does it: `phase1/run_v20_attribute.sh` (~2.9 h measured), 2×2 over {Z block: BART \| exact} × {X block: leftcens \| exact}, three `n` levels, with `ef_bart_ship` as the port-check control. **Registered prediction: the Z block dominates** (≥70%), because leftcens is approximately correct under a fork's linear-Gaussian exposure conditional while BART pays a nonparametric smoothing error at the n^−1/3 rate V18 measured. Note that V12/V13's exact Z draw is **wrong under a fork** — it omits p(logX1 \| Z1) and its draws are 32% too wide — hence `R/exact_fork.R`, verified in `test_v20_exact.R`. |
