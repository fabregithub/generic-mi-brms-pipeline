...
| **What a shippable Z-block fix would be** | ⚠️ **narrowed 2026-09-07 (V21): a fix exists in principle, but it is a TRADE and the deciding cell does not exist** | V21's ladder attributes **100%** of the bias to BART's *flexibility*: a correctly specified **estimated** linear draw is +0.26% / +0.58%, and so are improper and donor-matched versions. Properness moves the interval (`b` −12–16%), not the bias; the inner-FCS loop contributes nothing. **But both cells have a linear-Gaussian covariate conditional, so every parametric arm is correct by construction** — and R8 adopted BART precisely because a parametric Z block is misspecified when that conditional is non-linear (V6: `mice pmm` at −4.70%). Trading a 5% bias under linearity for a 5% bias under non-linearity is not a fix. `phase1/FINDINGS_v21.md`. |
| **Build a non-linear covariate conditional ON an X–Y path** | Open — **the deciding cell, and the next thing to build** | Nothing in the harness has one: `z_form = "nonlinear"` makes `Z1` a *descendant* of the exposures, which V17 showed is off-path, while the `fork`/`pipe` roles are linear-Gaussian by construction. So the one comparison that decides whether a parametric covariate draw is a fix or a different bug — parametric versus BART where the covariate conditional is genuinely non-linear *and* causally active — cannot be run. It needs a `fork`/`pipe` variant whose `Z1 → X1` or `X1 → Z1` arrow is non-linear; `simulate_complete()`'s role branch is where it goes, and appending a scenario keeps every existing cell bit-identical. Then re-run `run_v21_zdraw_ladder.sh` unchanged. |
| **Why V19's parametric imputers are biased when the property is fine** | Open — narrowed 2026-09-07 (V21) | V19 measured `micePmm` at +2.4 to +3.9% and `properZ` at +4.5 to +6.3%, both *asymptotic*; V21's parametric arms sit at ±0.6%. The boundary was registered before V21 ran, so this is a **located discrepancy, not a contradiction**: `z21_pmm` draws `Z1` alone on the correct formula with `Z2` exact, while `micePmm` imputes `Z1`, `Z2` **and `Y`** together with mice's own predictor matrix and iteration. Candidates: multi-target joint imputation, the predictor matrix, or `Y` as an imputation target. Cheap to test — extend the ladder to draw `Z2` by the same method as `Z1`, then to include `Y`. |
