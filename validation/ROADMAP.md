...
| **V24 has no arrow-absent control, so `u`'s role rests on a cross-instrument comparison** | Open — small and cheap, 2026-09-09 | V24 shows the linear-pipe cells carry 6–7% asymptotic bias with `u` = 0, and V23's `cv000` (`g ≡ 0`, also `u` = 0) reads −0.80% — which together say `u` cannot be the defect's generator, only its modifier. But `cv000` was measured on the pipeline arms and V24's cells on its own harness draw, so the contrast crosses instruments. **The fix is one scenario**: a `dag_*` cell with `delta_xz` = 0 (arrow absent, covariate still present and observed), run with the same six arms. It would separate "the child factor carries information that is being discarded" from "the child factor is misspecified", within one instrument. `THEORY.md` §6b. |
