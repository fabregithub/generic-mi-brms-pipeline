...
| **The Z-block scale defect's MAGNITUDE is not derived** | Open — 2026-09-10, and it is the honest gap in an otherwise closed item | The fix is derivation-backed (congeniality, `THEORY.md` §1 clause A) and demonstrated (four gaps → ≤0.20 pp). What is missing is a formula that PREDICTS the 3–5 pp rather than reproducing it. First attempt: for `L = logX₁ ~ N(m, s²)` and `R = e^L`, a linear predictor in `R` retains `Corr(L,R)² = s²/(e^{s²} − 1)` of the explainable variance — 0.470 (`zr_fork`, `s²` = 1.360) and 0.582 (`zr_pipe`, `s²` = 1.000). **Measured 0.339 and 0.447: a consistent −0.13 miss.** The error is identified, not mysterious: that is the marginal retained fraction, while the Z-block model conditions on `(logX₂, logX₃, Z₂, Y)` — and `Y` carries `logX₁` directly, so the relevant quantity is a partial correlation, under which `R = e^L` is no longer the exponential of the conditioning residual. Deriving the partial version, then propagating it through the `T₁ + T₂` decomposition (§0c) to a predicted bias, is what would make this a demonstrated derivation rather than a demonstrated fix. Algebra, not compute. |
