...
| **Build the DAG-factorised exposure draw** | Open — **the constructive successor to item 07**, specified 2026-09-09 | `THEORY.md` §6b derives the target from Bayes plus the DAG's factorisation: `p(x \| rest, x≤L) ∝ p(x \| Pa(X)) · ∏_{C∈Ch(X)} p(C \| x, ·) · 1{x≤L}`. `leftcens` implements the **fork** form only (every covariate a parent, no child factor) — exact for a fork (+0.28% measured), wrong for a pipe or collider whose child factor is non-linear. The algorithm is a grid inverse-CDF draw with **one added term**; the estimand does not change the target, only the analysis model applied afterwards. **It is needed in four cases, not two**: the fork is already correct, but pipe-direct, **pipe-total** and **collider** all need the child factor — including the two where `Z` is absent from the analysis model, because `Z` is a *child* of `X` and `p(Z\|x)` is observed information about `x` regardless. Dropping `Z` there does not remove the non-linearity, it moves it from `p(Z\|x)` into `p(Y\|x)` (clause A instead of clause B): measured +16.87% with `Z` in and +14.31% with it out, against **−0.26%** with the child factor on. **Design-tested with the child model supplied: +6.50% → −0.26% (direct) and +4.73% → −0.26% (total), bias/SE 0.96 → 0.04.** `phase1/derive_dag_draw.R`. **The remaining risk is entirely `p(Z\|x)` below the LOD, which §0b shows is not identifiable** — so it ships as a declared assumption with a sensitivity axis, not an estimate. |
