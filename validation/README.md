...
| [`THEORY.md`](THEORY.md) | **Why the pipeline fails where it fails.** What in the account is an identity, a claim, or a guess (§0); why the governing quantity is not identifiable (§0b); **an exact two-term decomposition of the bias, derived and verified to machine precision (§0c)** — which says congeniality is the `Y`-loading at which censoring attenuation and outcome borrowing cancel, and hence which way a wrong draw errs; the condition itself (§1), when a violation reaches an estimand and how the adjustment set follows from it (§1b, §1b(i)), **what a correct draw would be — the DAG-factorised target and its algorithm, design-tested (§6b)**, the derived mechanism behind the censored-exposure defect and the reason its law is quadratic (§3c), the two rates that make `bias/SE` the right currency and direction a first-class quantity (§4b), and an explicit list of what is *not* derived (§4) | Before designing a fix, quoting any magnitude, or when a new result looks surprising |
