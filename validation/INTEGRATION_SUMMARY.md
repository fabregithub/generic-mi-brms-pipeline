...
**As of 2026-08-29**, after V3 closed the mixture question, and v1.5.1 narrowed the Z-block inner-iteration default (V8), v1.5.0 adopted BART for the Z block (R8 closed) and the
...
| **NEW: substantive-model-compatible imputation** | The only remaining fix for the mixture path. The X block draws the censored exposure from a conditional linear in (Y, other X, Z); a mixture surface with curvature in X1 cannot be represented by it, and V3 measured the cost | Put the exposure–response *surface* into the imputation model, or use a joint model with the surface. Tractable in principle for a known parametric surface; the general case is research, not engineering | Large |
...
**Not claimable yet:** behaviour under MAR (as
opposed to MCAR) missingness; behaviour under a non-linear *outcome* model; that BART's
