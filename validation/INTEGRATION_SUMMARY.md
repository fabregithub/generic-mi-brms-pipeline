...
**As of 2026-08-31**, after V11 tested the imputer default against a non-linear outcome, V9 confirmed the mixture failure's cause, V3 closed the mixture question, and v1.5.1 narrowed the Z-block inner-iteration default (V8), v1.5.0 adopted BART for the Z block (R8 closed) and the
...
| **NEW: shape-aware Z-block draw** *(roadmap 08, pending V12)* | Every row-level imputer draws covariates as *(fitted conditional mean) + homoscedastic Gaussian noise*. That is exactly right under a linear outcome — `p(Z1 | Y, X)` then has constant SD and zero skew — and wrong under a non-linear one, where SD runs 0.626→1.216 and skew reaches −1.338. V11 measured ≈3 pp of bias for **all four** imputers (spread 0.44 pp) while the oracle moved 0.01 pp | Sample the conditional rather than assume it Gaussian. The 1-D grid sampler exists and is validated (`phase1/R/smc_impute.R`); the open question is obtaining the conditional inside the pipeline, where the outcome model is a `brms` fit rather than a known formula. **Conditional on V12** — if the shape contrast is small the mechanism is wrong and this closes unbuilt | Small–medium |
